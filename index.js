...
export default function handler(req, res) {
    ...
    const path = req.url?.replace('/api', '') || req.query?.path?.[0] || '';

    // GET /api/game - Get current game state
    if (req.method === 'GET' && (path === '/game' || path === '')) {
    ...
    if (req.method === 'POST' && path === '/assign-numbers') {
    ...
    if (req.method === 'POST' && path === '/update-teams') {
    ...
    if (req.method === 'POST' && path === '/set-score') {
